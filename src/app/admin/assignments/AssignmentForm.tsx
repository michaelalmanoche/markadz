...
const Assignments = () => {
  ...
  const handleAssign = async () => {
    try {
      ...
      toast.error('Failed to assign vans');
    }
  };
            ...
            <tr key={van.id} className="border-b">
              <td className="px-4 py-2" style={{ wordBreak: 'break-word' }}>
                ...
                <input
                  ...
                  onChange={(e) => handleVanSelection(van.id, e.target.checked ? van.id : '')}
                  disabled={!!isAssigned}
              ...
              <td className="px-4 py-2" style={{ wordBreak: 'break-word' }}>
                <select
                  ...
                  className="mt-1 ml-[-5rem] block w-[14rem] py-2 px-3 border text-center border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                >
