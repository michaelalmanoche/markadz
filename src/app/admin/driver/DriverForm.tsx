    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 409) {
          showToast("License number already registered", 'error');
        } else {
          showToast("Failed to register Driver", 'error');
        }
      }
    }
  };
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
    <div className="bg-red-100 p-4 rounded-lg shadow-lg w-full max-w-sm mx-auto">
      <div className="flex bg-red-100 rounded-lg p-4 mb-4 text-sm text-red-700" role="alert">
        ...
        <div>
          <span className="font-medium">Confirm Archive</span> Are you sure you want to archive this Driver?
        </div>
      </div>
